...
#define get_cli_no_opt_arg(cli_ctx,argv,argc) get_cli_no_opt_arg_full(cli_ctx,argv,argc,1)
char* get_cli_no_opt_arg_full (struct cli_ctx_t *cli_ctx, char **argv, int argc, int start)
{
    char *arg = NULL;

    for (int i=start; arg==NULL && i<argc; i++) {
        if (argv[i][0] == '-') {
